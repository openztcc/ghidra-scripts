...
with open(file_name, "r") as ida_export, open(log_file_name, "w") as ida_import_log:
    for line in ida_export:
        ...
        name = remove_text_inside_brackets(pieces[0], brackets="()").replace(" ", "")
        name_with_prototype = pieces[0]
        ...
        if func is not None:
            ...
            if old_name == name:
                add_plate_comment(address, name_with_prototype, ida_import_log)
                continue
            try:
                if name.startswith(".") and not old_name.startswith("FUN"):
                    log_line(ida_import_log, "Not renaming {} to {}".format(old_name, name))
                    continue
                func.setName(name, SourceType.USER_DEFINED)
                log_line(ida_import_log, "Renamed function {} to {} at address {}".format(old_name, name, address))
                add_plate_comment(address, name_with_prototype, ida_import_log)
            except Exception as e:
                print("Failed to rename function from {} to {} because {}".format(old_name, name, e))
                log_line(ida_import_log, "Failed to rename function from {} to {}".format(old_name, name))
                continue
        else:
            try:
                func = createFunction(address, name)
                log_line(ida_import_log, "Created function {} at address {}".format(name, address))
                add_plate_comment(address, name_with_prototype, ida_import_log)
            except:
                print("Failed to create function {}".format(name))
                log_line(ida_import_log, "Failed to create function {}".format(name))
                continue
